...
public class ImportsController : ControllerBase
{
    ...
    public async Task<IActionResult> UploadExcel([FromForm] ImportExcelRequest request, CancellationToken cancellationToken)
    {
        ...
        {
            var internalCode = NormalizeCodeOrNull(GetCellValue(row, headers, "codigo navi", "codigo interno", "internal code", "c�digo interno", "codigo herramienta", "code"));
            var fenixCode = NormalizeCodeOrNull(GetCellValue(row, headers, "codigo fenix", "fenix code", "codigo fenix365", "c�digo fenix365"));
            ...
            var toolName = GetCellValue(row, headers, "nombre", "herramienta", "tool name", "descripcion", "descripci�n");
        ...
        importBatch.Summary = $"Filas: {importBatch.TotalRows}. V�lidas: {importBatch.ValidRows}. Errores: {importBatch.ErrorRows}. Duplicados/Existentes: {importBatch.DuplicateRows}.";
    ...
    private async Task AnalyzeImportRowAsync(ImportRow row, CancellationToken cancellationToken)
    {
        ...
        {
            row.ResultStatus = "Error";
            row.Message = "La fila no tiene c�digo interno, c�digo Fenix365, activo fijo ni serial.";
            return;
        }
        ...
        if (existingTool is null)
        {
            ...
            row.Message = "Herramienta candidata para creaci�n. No existe coincidencia en NAVI.";
            return;
        }
            ...
            && string.Equals(existingTool.InternalCode, row.InternalCode, StringComparison.OrdinalIgnoreCase))
        {
            ...
            row.Message = differences.Count == 0
                ? $"La herramienta ya existe en NAVI con c�digo {existingTool.InternalCode}."
