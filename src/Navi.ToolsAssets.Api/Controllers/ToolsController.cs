...
public class ToolsController : ControllerBase
{
    ...
    public async Task<IActionResult> CreateTool([FromBody] CreateToolRequest request)
    {
        ...
        var tool = new ToolAsset
        {
            ...
            ShelfLocation = NormalizeShelfLocation(request.ShelfLocation),
    ...
    public async Task<IActionResult> UpdateTool(Guid id, [FromBody] UpdateToolRequest request)
    {
        ...
        tool.ShelfLocation = NormalizeShelfLocation(request.ShelfLocation);
    ...
    public async Task<IActionResult> UpdateAvailabilityLocation(Guid id, [FromBody] UpdateAvailabilityLocationRequest request)
    {
        ...
        var previousValue = $"Sede={tool.Branch?.Code}; Ubicación={tool.Location?.Code}; Estantería={tool.ShelfLocation}; Estado={tool.OperationalStatus}; Responsable={tool.ResponsiblePerson?.FullName}";
        ...
        var newValue = $"Sede={branch.Code}; Ubicación={location?.Code}; Estantería={tool.ShelfLocation}; Estado={tool.OperationalStatus}; Responsable={tool.ResponsiblePerson?.FullName}";
    ...
    public async Task<IActionResult> AssignFixedAsset(Guid id, [FromBody] AssignFixedAssetRequest request)
    {
        ...
        else if (assignmentType.Equals("Warehouse", StringComparison.OrdinalIgnoreCase))
        {
            ...
            var previousValue = $"Responsable={tool.ResponsiblePerson?.FullName}; Sede={tool.Branch?.Code}; Ubicación={tool.Location?.Code}; Estantería={tool.ShelfLocation}; Estado={tool.OperationalStatus}; Custodia={tool.CustodyStatus}";
    ...
    private static string? NormalizeShelfLocation(string? value)
    {
        if (string.IsNullOrWhiteSpace(value))
        {
            return null;
