...
`NaviMasterDbContext` tiene migraciones independientes en
`src/Navi.ToolsAssets.Infrastructure/Tenancy/Migrations` e historial en el esquema `Master`.

```bash
NAVI_MASTER_DB_CONNECTION='<conexión-segura>' dotnet ef migrations script \
  --context NaviMasterDbContext \
  --project src/Navi.ToolsAssets.Infrastructure \
  --startup-project src/Navi.ToolsAssets.Infrastructure \
  --idempotent
```
